...
import {Api} from '../../Api'
class InterNationalItems extends Component {
  constructor(props){
    super(props)
    this.state = {
      show :false,
  ...
  DeletePost =async () => {
    ...
    if(response.status === 204){
      this.props.history.push('/internationals')
    }
  ...
  edit = () => {
    this.props.history.push({
      pathname:`/editInterNational/${this.state.id}`,
      state: {
      ...
      <div className="container mt-5 pt-5 mb-3 pb-3">
        <div className="row">
          <div className="col  col-sm-2 col-md-2 col-lg-2 col-md-2 ">
            <Link className="btn btn-outline-danger"  to="/international">Back</Link>
          </div>
...
export default withRouter(InterNationalItems)